//do this outside of this file to initiate it
/*
//Stuff for the fast I420->rgb 32 conversion
#define RGBA32
#include "colorspace_template.h"
#undef RGBA32
#define BGRA32
#include "colorspace_template.h"
#undef BGRA32
...
//these are the necessary conditions for these functions to work
/*	if (src_format.colorspace							==	YUV601 
		&& src_format.organisation						==	PLANAR
		&& src_format.active_channels					==	3
		&& src_format.chromalayout.horz_position		==	1
		&& src_format.chromalayout.horz_subs_factor		==	2
		&& src_format.chromalayout.horz_support_end		==	4
		&& src_format.chromalayout.horz_support_start	==	0
		&& src_format.chromalayout.vert_position		==	2
		&& src_format.chromalayout.vert_subs_factor		==	2
		&& src_format.chromalayout.vert_support_end		==	4
		&& src_format.chromalayout.vert_support_start	==	0	 
		&& dst_format.colorspace						==	RGB
		&& dst_format.organisation						==	PACKED
		&& dst_format.pixel_repeat_period				==	1
		&& dst_format.total_bpp							==	32
		//&& dst_format.active_bpp						==	32
		//&& dst_format.active_channels					==	4
		&& bDoAdaptiveInterpolation						==	0
		&& 1)
	{
		colorconverter->conversion					=	I420_TO_RGB;
		colorconverter->buffer1_size				=	0;
		//colorconverter->buffer1					=	malloc(colorconverter->buffer1_size);
		colorconverter->buffer2_size				=	0;
		//colorconverter->buffer2					=	malloc(colorconverter->buffer1_size);
	}*/


//do this outside this file to call it
	/*if (colorconverter->conversion == I420_TO_RGB)
	{
		...
		if (colorconverter->dst_format.total_bpp == 32)
		{
			if (	colorconverter->dst_format.ch0.mask1	==	0x000000FF
				&& colorconverter->dst_format.ch1.mask1		==	0x0000FF00
				&& colorconverter->dst_format.ch2.mask1		==	0x00FF0000)
				//&& colorconverter->dst_format.ch3.mask1		==	0xFF000000)
			{
				yuv420_to_abgr32(yp, up, vp, rgbp, height, width);
			}
			else if(	colorconverter->dst_format.ch2.mask1	==	0x000000FF
				&&	colorconverter->dst_format.ch1.mask1		==	0x0000FF00
				&&	colorconverter->dst_format.ch0.mask1		==	0x00FF0000)
				//&&	colorconverter->dst_format.ch3.mask1		==	0xFF000000)
			{
				yuv420_to_argb32(yp, up, vp, rgbp, height, width);
			}
			else if(	//colorconverter->dst_format.ch3.mask1	==	0x000000FF
					colorconverter->dst_format.ch2.mask1		==	0x0000FF00
				&&	colorconverter->dst_format.ch1.mask1		==	0x00FF0000
				&&	colorconverter->dst_format.ch0.mask1		==	0xFF000000)
			{
				yuv420_to_rgba32(yp, up, vp, rgbp, height, width);
			}
			else if(	//colorconverter->dst_format.ch3.mask1	==	0x000000FF
					colorconverter->dst_format.ch0.mask1		==	0x0000FF00
				&&	colorconverter->dst_format.ch1.mask1		==	0x00FF0000
				&&	colorconverter->dst_format.ch2.mask1		==	0xFF000000)
			{
				yuv420_to_bgra32(yp, up, vp, rgbp, height, width);
			}
		}
	}*/

//now this actual file
#include <emmintrin.h>
